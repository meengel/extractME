...
from .EmpiricalDist import EmpDist, EmpDigest
...
class ERADist(object):
    ...
    The following distribution types are available:
      ...
      Empirical Digest:           Obj = ERADist('empiricaldigest', 'DATA', [X, weights, [nApprox, mode, eps, pdfMethod, pdfPoints, kdeKwargsDict]])
    """
#%%
    def __init__(self, name, opt, val=[0, 1], ID=False):
        ...
        # if the distribution is to be fitted to a data vector
            ...
            if name.lower() == "beta":
                ...
                self.Par.update(val[7])
                self.Dist = EmpDigest(data=X, weights=weights, **self.Par) # implemented by Michael Engel
